...
gc::future<void> SendMessages(pubsub::Publisher publisher) {
  std::vector<gc::future<gc::StatusOr<std::string>>> pending;
  std::generate_n(std::back_inserter(pending), 5, [&publisher]() {
    return publisher.Publish(pubsub::MessageBuilder().SetData("Hi!").Build());
  });
  for (auto& f : pending) {
    auto id = co_await f;
    if (!id) {
...
int main(int argc, char* argv[]) try {
  ...
  // In this example, we will send send some messages and block until they are
  // all sent.
  SendMessages(std::move(publisher)).get();
